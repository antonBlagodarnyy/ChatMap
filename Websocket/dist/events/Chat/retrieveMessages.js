...
routeMessagesRetrieve.get("/retrieve/:receiver", jwtCheck, async (req, res) => {
    ...
    if (token)
        try {
            const routeMessagesRetrieveRes = await axios.get(`${process.env.MESSAGE_URL}/message/retrieve`, {
                headers: {
            ...
            if (usersIds.length != 0) {
                const sendersUsernamesRes = await axios.get(`${process.env.PROFILE_URL}/profile/usernamesById`, {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: token,
                    },
...
//# sourceMappingURL=retrieveMessages.js.map