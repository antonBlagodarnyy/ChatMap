...
routerCreateUser.post("/create", async (req, res) => {
    //Create the user
    try {
        const authResponse = await axios.post(`${process.env.AUTH_URL}/auth/signup`, req.body, {
            headers: {
        ...
        const profileResponse = await axios.post(`${process.env.PROFILE_URL}/profile/create`, {
        ...
        if (err.config.url.includes("/profile/create")) {
            ...
            try {
                await axios.post(`${process.env.AUTH_URL}/protected/delete`, {}, { headers: { Authorization: "Bearer " + jwt } });
            }
            catch (err) {
                console.log("WARNING: Could not create profile and could not rollback" + err);
            }
        }
